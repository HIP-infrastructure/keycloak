...
def main():
    ...
    argsparser.add_argument('-f', '--force-role-creation', dest='force_creation', default=False, help='Create role if doesn not exists', type=bool)
    ...
    try:
        ...
        try:
            ...
            if args['force_creation']:
                ...
                print('Role <%s> created' %(args['user_role']))
            else:
        ...
        keycloak_adm.assign_realm_roles(user_id=wanted_user_id, roles=[{"id": wanted_role['id'], "name": wanted_role['name']}])
        print('Role <%s> added to user <%s>' %(args['user_role'], args['user_name']))
